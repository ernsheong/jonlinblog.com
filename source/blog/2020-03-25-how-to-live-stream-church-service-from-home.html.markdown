...
Fortunately, consumer technology has matured by leaps and bounds and there are other viable solutions. While this guide tries hard to make possible for anyone to achieve a livestream, in practice you probably need to be a bit comfortable tinkering with some hardware and software, so share this blog post with the technical guy in your church.
...
OBS is a desktop-based software that basically captures your screen(s) and your computer audio all at the same time, and channels it to somewhere else (Facebook Live, YouTube Live, Twitch, etc.). You can even just use OBS to so a video recording, without streaming live. In our setup, we will capture what is shown on the two external monitors as well as computer audio, and mix them together into a stream, which we will then send to Facebook Live.
...
6. You may also choose to embed your live view into your church website. Copy and paste the iframe code. While this is quick and easy, the iframe will not show up nicely in mobile (the video will fit nicely, but it will have a huge chunk of white space below it because the iframe height is hardcoded). If you are particular, you can go to [https://developers.facebook.com/docs/plugins/embedded-video-player/](https://developers.facebook.com/docs/plugins/embedded-video-player/) and figure out how to embed the video in a much nicer way.
